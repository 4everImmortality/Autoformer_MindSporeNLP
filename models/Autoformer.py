...
class Model(nn.Cell):
    ...
    def construct(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        mean = ops.mean(x_enc, axis=1).unsqueeze(1).repeat(1, self.pred_len, 1)
        zeros = ops.Zeros()((x_dec.shape[0], self.pred_len, x_dec.shape[2]), ms.float32)
        seasonal_init, trend_init = self.decomp(x_enc)
        trend_init = ops.Concat(axis=1)((trend_init[:, -self.label_len:, :], mean))
        seasonal_init = ops.Concat(axis=1)((seasonal_init[:, -self.label_len:, :], zeros))
