...
class AutoCorrelation(nn.Cell):
    ...
    def time_delay_agg_training(self, values, corr):
        ...
        mean_value = ops.ReduceMean()(corr, (1, 2))
        ...
        # aggregation
        tmp_values = values
        delays_agg = ops.ZerosLike()(values).astype(ms.float32)
        for i in range(top_k):
            pattern = ops.Roll(shifts=-int(index[i]), axes=-1)(tmp_values)
            delays_agg += pattern * \
                          (ops.ExpandDims()(tmp_corr[:, i], (1, 2, 3)).repeat(head, axis=1) * 
                           ops.ExpandDims()(ops.ExpandDims()(ops.ExpandDims()(tmp_corr[:, i], 1), 1), 1).repeat(channel, axis=2) *
                           ops.ExpandDims()(ops.ExpandDims()(ops.ExpandDims()(tmp_corr[:, i], 1), 1), 1).repeat(length, axis=3))
        return delays_agg
    ...
    def time_delay_agg_inference(self, values, corr):
        ...
        length = values.shape[3]
        # index init
        init_index = ops.Tile()(ops.ExpandDims()(ops.Range()(0, length), (0, 1, 2, 3)), (batch, head, channel, 1))
        # find top k
        top_k = int(self.factor * np.log(length))
        mean_value = ops.ReduceMean()(corr, (1, 2))
        weights, delay = ops.TopK(sorted=True)(mean_value, top_k)
        # update corr
        tmp_corr = ops.Softmax(axis=-1)(weights)
        # aggregation
        ...
        delays_agg = ops.ZerosLike()(values).astype(ms.float32)
        for i in range(top_k):
            tmp_delay = init_index + ops.ExpandDims()(delay[:, i], (1, 2, 3)).repeat(head, axis=1) * \
                        ops.ExpandDims()(ops.ExpandDims()(ops.ExpandDims()(delay[:, i], 1), 1), 1).repeat(channel, axis=2) * \
                        ops.ExpandDims()(ops.ExpandDims()(ops.ExpandDims()(delay[:, i], 1), 1), 1).repeat(length, axis=3)
            pattern = ops.GatherND()(tmp_values, tmp_delay.astype(ms.int32))
            delays_agg += pattern * (ops.ExpandDims()(tmp_corr[:, i], (1, 2, 3)).repeat(head, axis=1) * 
                                     ops.ExpandDims()(ops.ExpandDims()(ops.ExpandDims()(tmp_corr[:, i], 1), 1), 1).repeat(channel, axis=2) *
                                     ops.ExpandDims()(ops.ExpandDims()(ops.ExpandDims()(tmp_corr[:, i], 1), 1), 1).repeat(length, axis=3))
        return delays_agg
    ...
    def time_delay_agg_full(self, values, corr):
        ...
        # find top k
        top_k = int(self.factor * np.log(length))
        weights, delay = ops.TopK(sorted=True)(corr, top_k)
        # update corr
        tmp_corr = ops.Softmax(axis=-1)(weights)
        # aggregation
        tmp_values = ops.Tile()(values, (1, 1, 1, 2))
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        q_fft = ops.RFFT()(queries.transpose(0, 2, 3, 1), normalized=True)
        k_fft = ops.RFFT()(keys.transpose(0, 2, 3, 1), normalized=True)
        res = q_fft * ops.Conj()(k_fft)
        corr = ops.IRFFT()(res, n=L, normalized=True)
